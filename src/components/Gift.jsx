...
const Gift = () => {
    ...
    if (!isGameWon && attempts < 3) {
      if (attempts < 2) {
        ...
        if (restartCount < 2) {
          ...
          setTimeout(() => openWinnerModal(), 1500);
        }
        setIsGameWon(false);
      }
    } else {
      alert('Игра окончена. Вы использовали все 3 попытки.');
    }
  };
  ...
  return (
    <div>
      <div className="grid grid-cols-3 mx-auto max-w-xl">
        {gifts.map((gift, index) => (
          <img key={index} src={gift} className="w-[96px] h-[96px] cursor-pointer" alt={`Подарок ${index + 1}`} onClick={() => handleClick(index)} />
        ))}
      </div>
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              <Transition.Child
                ...
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="mt-4">
                    <button
                      className="bg-[#4B94F7] py-2 px-5 w-full rounded-full text-base text-white"
                      onClick={() => {
                        closeModal();
                        resetGame();
                      }}>
